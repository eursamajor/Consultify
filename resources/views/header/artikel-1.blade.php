<!DOCTYPE html>
<html lang="zxx">
<head>
  <meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
  <meta name="description" content="Orbitor,business,company,agency,modern,bootstrap4,tech,software">
  <meta name="author" content="themefisher.com">

  <title>Consultify - Find your happiness here</title>

  <!-- Favicon -->
  <link rel="shortcut icon" type="image/x-icon" href="/images/favicon.ico" />

  <!-- bootstrap.min css -->
  <link rel="stylesheet" href="{{ asset('assets/plugins/bootstrap/css/bootstrap.min.css') }}">
  <!-- Icon Font Css -->
  <link rel="stylesheet" href="{{ asset('assets/plugins/icofont/icofont.min.css') }}">
  <!-- Slick Slider  CSS -->
  <link rel="stylesheet" href="{{ asset('assets/plugins/slick-carousel/slick/slick.css') }}">
  <link rel="stylesheet" href="{{ asset('assets/plugins/slick-carousel/slick/slick-theme.css') }}">

  <!-- Main Stylesheet -->
  <link rel="stylesheet" href="{{ asset('assets/css/style.css') }}">


</head>

<body id="top">

<header>
	<div class="header-top-bar">
		<div class="container">
			<div class="row align-items-center">
				<div class="col-lg-6">
					<ul class="top-bar-info list-inline-item pl-0 mb-0">
						<li class="list-inline-item"><a href="mailto:support@gmail.com"><i class="icofont-support-faq mr-2"></i>support@consultify.com</a></li>
						<li class="list-inline-item"><i class="icofont-location-pin mr-2"></i>Bandung, Indonesia </li>
					</ul>
				</div>
				<div class="col-lg-6">
					<div class="text-lg-right top-right-bar mt-2 mt-lg-0">
						<a href="tel:+021-xxx-xxx" >
							<span>Call Now : </span>
							<span class="h4">021-xxx-xxx</span>
						</a>
					</div>
				</div>
			</div>
		</div>
	</div>
	<nav class="navbar navbar-expand-lg navigation" id="navbar">
		<div class="container">
		<a class="navbar-brand" href="about.php">Artikel<span>.</span></a>

		  	<button class="navbar-toggler collapsed" type="button" data-toggle="collapse" data-target="#navbarmain" aria-controls="navbarmain" aria-expanded="false" aria-label="Toggle navigation">
			<span class="icofont-navigation-menu"></span>
		  </button>
	  
		  <div class="collapse navbar-collapse" id="navbarmain">
			<ul class="navbar-nav ml-auto">
			  <li class="nav-item active">
				<a class="nav-link" href="about.php">Home</a>
			  </li>
				<li class="nav-item"><a class="nav-link" href="artikel.php">Artikel</a></li>
				<li class="nav-item"><a class="nav-link" href="konselor.php">Konselor</a></li>
			   <li class="nav-item"><a class="nav-link" href="riwayat.php">Riwayat</a></li>
			   <li class="nav-item"><a class="nav-link" href="login.php">Login</a></li>
			</ul>
		  </div>
		</div>
	</nav>
</header>
	


<section class="page-title bg-1">
  <div class="overlay"></div>
  <div class="container">
    <div class="row">
      <div class="col-md-12">
        <div class="block text-center">
          <span class="text-white">detail</span>
          <h1 class="text-capitalize mb-5 text-lg">Artikel</h1>

          <!-- <ul class="list-inline breadcumb-nav">
            <li class="list-inline-item"><a href="index.html" class="text-white">Home</a></li>
            <li class="list-inline-item"><span class="text-white">/</span></li>
            <li class="list-inline-item"><a href="#" class="text-white-50">News details</a></li>
          </ul> -->
        </div>
      </div>
    </div>
  </div>
</section>



<section class="section blog-wrap">
    <div class="container">
        <div class="row">
            <div class="col-lg-8">
                <div class="row">
	<div class="col-lg-12 mb-5">
		<div class="single-blog-item">
			<img src="images/blog/blog-1.jpg" alt="" class="img-fluid">

			<div class="blog-item-content mt-5">
				<div class="blog-item-meta mb-3">
					<span class="text-color-2 text-capitalize mr-3"><i class="icofont-book-mark mr-2"></i> simpan</span>
					<span class="text-muted text-capitalize mr-3"><i class="icofont-comment mr-2"></i>5 Komentar</span>
					<span class="text-black text-capitalize mr-3"><i class="icofont-calendar mr-2"></i> 28 Januari 2019</span>
				</div> 

				<h2 class="mb-4 text-md"><a href="blog-single.html">Mari konsultasi kesehatan setiap tahun</a></h2>

				<p class="lead mb-4">Kesehatan adalah pilar penting dalam kehidupan, karena untuk apa kaya dan menawan tetapi kita tidak sehat. Maka dari itu, konsultasi kesehatan merupakan alternatif untuk menjaga kesehatan secara berkala.</p>

				<p>Mari konsultasi kesehatan setiap tahun. Ini bukan hanya sebuah saran, tetapi sebuah kebutuhan yang penting untuk kesejahteraan kita. Dalam dunia yang penuh dengan tuntutan dan tekanan, kesehatan seringkali terabaikan. Kita terjebak dalam rutinitas sehari-hari dan sering kali lupa bahwa tubuh kita adalah aset terpenting yang kita miliki. Konsultasi kesehatan tahunan merupakan langkah penting dalam pemeliharaan kesehatan kita. Ini memberi kesempatan bagi kita untuk mengecek kondisi tubuh secara menyeluruh, dari pemeriksaan fisik hingga pemeriksaan laboratorium. Lebih dari itu, ini adalah waktu yang tepat untuk berbicara dengan dokter tentang kekhawatiran atau perubahan yang mungkin kita alami.</p>

				<blockquote class="quote">
					"Harta yang paling berharga adalah kesehatan."
				</blockquote>

				
				<p class="lead mb-4 font-weight-normal text-black">Harta yang paling berharga adalah kesehatan, sebuah perhiasan tak ternilai yang melebihi segala kekayaan, karena tanpa kesehatan, segala impian dan aspirasi menjadi tak berarti.</p>

				<p>Dengan demikian, kita tidak hanya memelihara kesehatan fisik, tetapi juga kesehatan mental dan emosional. Jadi, mari kita buat komitmen untuk konsultasi kesehatan setiap tahun sebagai investasi dalam diri kita sendiri, untuk masa depan yang lebih sehat dan lebih bahagia.






</p>

				<div class="mt-5 clearfix">
				    <ul class="float-left list-inline tag-option"> 
				    	<li class="list-inline-item"><a href="#">Karir</a></li>
				    	<li class="list-inline-item"><a href="#">Obat</a></li>
				    	<li class="list-inline-item"><a href="#">Konsultasi</a></li>
				   	</ul>        

				    <ul class="float-right list-inline">
				        <li class="list-inline-item"> Share: </li>
				        <li class="list-inline-item"><a href="#" target="_blank"><i class="icofont-facebook" aria-hidden="true"></i></a></li>
				        <li class="list-inline-item"><a href="#" target="_blank"><i class="icofont-twitter" aria-hidden="true"></i></a></li>
				        <li class="list-inline-item"><a href="#" target="_blank"><i class="icofont-pinterest" aria-hidden="true"></i></a></li>
				        <li class="list-inline-item"><a href="#" target="_blank"><i class="icofont-linkedin" aria-hidden="true"></i></a></li>
				    </ul>
			    </div>
			</div>
		</div>
	</div>

	<div class="col-lg-12">
		<div class="comment-area mt-4 mb-5">
			<h4 class="mb-4">2 Komentar... </h4>
			<ul class="comment-tree list-unstyled">
				<li class="mb-5">
					<div class="comment-area-box">
						<div class="comment-thumb float-left">
							<img alt="" src="images/blog/testimonial1.jpg" class="img-fluid">
						</div>

						<div class="comment-info">
							<h5 class="mb-1">Supriyadi</h5>
							<span>Jakarta</span>
							<span class="date-comm">| diunggah 7 April , 2019</span>
						</div>
						<div class="comment-meta mt-2">
							<a href="#"><i class="icofont-reply mr-2 text-muted"></i>Balasan</a>
						</div>

						<div class="comment-content mt-3">
							<p>Benar sekali, kesehatan memang tidak ternilai harganya. Tanpa kesehatan yang baik, semua yang kita miliki tidak bisa dinikmati sepenuhnya. Ini mengingatkan kita semua untuk selalu menjaga kesehatan sebagai prioritas utama dalam hidup. </p>
						</div>
					</div>
				</li>

				<li>
					<div class="comment-area-box">
						<div class="comment-thumb float-left">
							<img alt="" src="images/blog/testimonial2.jpg" class="img-fluid">
						</div>

						<div class="comment-info">
							<h5 class="mb-1">Sri Mulyani</h5>
							<span>Bandung</span>
							<span class="date-comm">| Diunggah 7 Juni, 2019</span>
						</div>

						<div class="comment-meta mt-2">
							<a href="#"><i class="icofont-reply mr-2 text-muted"></i>Balasan </a>
						</div>

						<div class="comment-content mt-3">
							<p>Pernyataan ini sangat menggugah! Sering kali kita terjebak dalam mengejar kekayaan materi, namun lupa bahwa kesehatan adalah aset terpenting. Ini adalah pengingat yang baik bahwa menjaga kesehatan adalah investasi terbaik yang bisa kita lakukan </p>
						</div>
					</div>
				</li>
			</ul>
		</div>
	</div>


	<div class="col-lg-12">
		<form class="comment-form my-5" id="comment-form">
			<h4 class="mb-4">Tulis Komentar</h4>
			<div class="row">
				<div class="col-md-6">
					<div class="form-group">
						<input class="form-control" type="text" name="name" id="name" placeholder="Name:">
					</div>
				</div>
				<div class="col-md-6">
					<div class="form-group">
						<input class="form-control" type="text" name="mail" id="mail" placeholder="Email:">
					</div>
				</div>
			</div>


			<textarea class="form-control mb-4" name="comment" id="comment" cols="30" rows="5" placeholder="Komentar"></textarea>

			<input class="btn btn-main-2 btn-round-full" type="submit" name="submit-contact" id="submit_contact" value="Kirim Pesan">
		</form>
	</div>
</div>
</div>
            <div class="col-lg-4">
                <div class="sidebar-wrap pl-lg-4 mt-5 mt-lg-0">
	<div class="sidebar-widget search  mb-3 ">
		<h5>Cari disini</h5>
		<form action="#" class="search-form">
			<input type="text" class="form-control" placeholder="Cari">
			<i class="ti-search"></i>
		</form>
	</div>


	<div class="sidebar-widget latest-post mb-3">
		<h5>Postingan Populer</h5>

        <div class="py-2">
        	<span class="text-sm text-muted">03 Mar 2018</span>
            <h6 class="my-2"><a href="#">Pentingnya air yang sehat dalam kehidupan.</a></h6>
        </div>

        <div class="py-2">
       		<span class="text-sm text-muted">07 Mar 2021</span>
            <h6 class="my-2"><a href="#">Keluarga Harmonis dengan lakukan ritual ini.</a></h6>
        </div>

        <div class="py-2">
        	<span class="text-sm text-muted">03 Mei 2019</span>
            <h6 class="my-2"><a href="#">Olahraga sebelum berkerja dapat meningkatkan mood.</a></h6>
        </div>
	</div>

	<div class="sidebar-widget category mb-3">
		<h5 class="mb-4">Kategori</h5>

		<ul class="list-unstyled">
		  <li class="align-items-center">
		    <a href="#">Kesehatan</a>
		    <span>(14)</span>
		  </li>
		  <li class="align-items-center">
		    <a href="#">Habbits</a>
		    <span>(2)</span>
		  </li>
		  <li class="align-items-center">
		    <a href="#">Cinta</a>
		    <span>(10)</span>
		  </li>
		  <li class="align-items-center">
		    <a href="#">Konseling gratis</a>
		    <span>(5)</span>
		  </li>
		  <li class="align-items-center">
		    <a href="#">Pekerjaan</a>
		    <span>(5)</span>
		  </li>
		</ul>
	</div>


	<div class="sidebar-widget tags mb-3">
		<h5 class="mb-4">Tags</h5>

		<a href="#">Kesehatan</a>
		<a href="#">Pekerjaan</a>
		<a href="#">Karir</a>
		<a href="#">Obat</a>
		<a href="#">Habbits</a>
		<a href="#">Marketing</a>
		<a href="#">Sosial Media</a>
		<a href="#">Branding</a>
		<a href="#">Mental health</a>
	</div>


	<div class="sidebar-widget schedule-widget mb-3">
		<h5 class="mb-4">Jam Kerja</h5>

		<ul class="list-unstyled">
		  <li class="d-flex justify-content-between align-items-center">
		    <a href="#">Senin - Rabu</a>
		    <span>8:00 - 17:00</span>
		  </li>
		  <li class="d-flex justify-content-between align-items-center">
		    <a href="#">Kamis - Jumat</a>
		    <span>9:00 - 17:00</span>
		  </li>
		  <li class="d-flex justify-content-between align-items-center">
		    <a href="#">Sabtu - Minggu</a>
		    <span>10:00 - 17:00</span>
		  </li>
		</ul>

		<div class="sidebar-contatct-info mt-4">
			<p class="mb-0">Bantuan Darurat</p>
			<h3>1-800-700-6200</h3>
		</div>
	</div>

</div>

</div>
            </div>   
        </div>
    </div>
</section>


<footer class="footer section gray-bg">
	<div class="container">
		<div class="row">
			<div class="col-lg-4 mr-auto col-sm-6">
				<div class="widget mb-5 mb-lg-0">
					<div class="logo mb-4">
						<img src="assets/images/consultify.png" alt="" class="img-fluid">
					</div>
					<p>Kunjungi media sosial kami untuk dapat melihat info menarik lainnya tentang consultify</p>

					<ul class="list-inline footer-socials mt-4">
						<li class="list-inline-item"><a href="https://www.facebook.com/consultify"><i class="icofont-facebook"></i></a></li>
						<li class="list-inline-item"><a href="https://twitter.com/themefisher"><i class="icofont-twitter"></i></a></li>
						<li class="list-inline-item"><a href="https://www.pinterest.com/themefisher/"><i class="icofont-linkedin"></i></a></li>
					</ul>
				</div>
			</div>

			<div class="col-lg-2 col-md-6 col-sm-6">
				<div class="widget mb-5 mb-lg-0">
			

					
				</div>
			</div>

			<div class="col-lg-2 col-md-6 col-sm-6">
				<div class="widget mb-5 mb-lg-0">
					
				</div>
			</div>

			<div class="col-lg-3 col-md-6 col-sm-6">
				<div class="widget widget-contact mb-5 mb-lg-0">
					<h4 class="text-capitalize mb-3">Get in Touch</h4>
					<div class="divider mb-4"></div>

					<div class="footer-contact-block mb-4">
						<div class="icon d-flex align-items-center">
							<i class="icofont-email mr-3"></i>
							<span class="h6 mb-0">Layanan tersedia 24 jam</span>
						</div>
						<h4 class="mt-2"><a href="tel:+0813-8889-4512">hermawanalghozy@gmail.com</a></h4>
					</div>

					<div class="footer-contact-block">
						<div class="icon d-flex align-items-center">
							<i class="icofont-support mr-3"></i>
							<span class="h6 mb-0">Layanan Senin s/d Jumat : 08:30 - 18:00</span>
						</div>
						<h4 class="mt-2"><a href="tel:+23-345-67890">+23-456-6588</a></h4>
					</div>
				</div>
			</div>
		</div>
		
		<!-- <div class="footer-btm py-4 mt-5">
			<div class="row align-items-center justify-content-between">
				<div class="col-lg-6">
					<div class="copyright">
						&copy; Copyright Reserved to <span class="text-color">Novena</span> by <a href="https://themefisher.com/" target="_blank">Themefisher</a>
					</div>
				</div> -->
				<!-- <div class="col-lg-6">
					<div class="subscribe-form text-lg-right mt-5 mt-lg-0">
						<form action="#" class="subscribe">
							<input type="text" class="form-control" placeholder="Your Email address">
							<a href="#" class="btn btn-main-2 btn-round-full">Subscribe</a>
						</form>
					</div>
				</div>
			</div>

			<div class="row">
				<div class="col-lg-4">
					<a class="backtop js-scroll-trigger" href="#top">
						<i class="icofont-long-arrow-up"></i>
					</a>
				</div>
			</div>
		</div>
	</div>
	<div class="col-lg-12">
		<div class="copyright text-lg-center">
	  Distributed by <a href="https://themewagon.com/" target="_blank">Themewagon</a>
		</div>
	</div>
</footer> -->
   

    <!-- 
    Essential Scripts
    =====================================-->

    
     <!-- Main jQuery -->
     <script src="plugins/jquery/jquery.js"></script>
    <!-- Bootstrap 4.3.2 -->
    <script src="plugins/bootstrap/js/popper.js"></script>
    <script src="plugins/bootstrap/js/bootstrap.min.js"></script>
    <script src="plugins/counterup/jquery.easing.js"></script>
    <!-- Slick Slider -->
    <script src="plugins/slick-carousel/slick/slick.min.js"></script>
    <!-- Counterup -->
    <script src="plugins/counterup/jquery.waypoints.min.js"></script>
    
    <script src="plugins/shuffle/shuffle.min.js"></script>
    <script src="plugins/counterup/jquery.counterup.min.js"></script>
    <!-- Google Map -->
    <script src="plugins/google-map/map.js"></script>
    <script src="https://maps.googleapis.com/maps/api/js?key=AIzaSyAkeLMlsiwzp6b3Gnaxd86lvakimwGA6UA&callback=initMap"></script>    
    
    <script src="js/script.js"></script>
    <script src="js/contact.js"></script>

  </body>
  </html>